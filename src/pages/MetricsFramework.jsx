...
import MetricsStart from '../components/metrics/MetricsStart'
import MetricsForm from '../components/metrics/MetricsForm'
import MetricsLoading from '../components/metrics/MetricsLoading'
import MetricsPreview from '../components/metrics/MetricsPreview'
import MetricsOutput from '../components/metrics/MetricsOutput'
...
export default function MetricsFramework() {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      const res = await fetch('/api/generate-metrics-framework', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-bg text-text">
      <div className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md bg-bg/80 border-b border-border">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <span className="text-muted text-xs">AI Metrics Framework · Free</span>
        </div>
      </div>

      <div className="pt-16">
        {stage === STAGES.start && <MetricsStart onStart={() => setStage(STAGES.form)} />}
        {stage === STAGES.form && (
        ...
        {stage === STAGES.loading && <MetricsLoading />}
        {stage === STAGES.preview && result && formData && (
          <MetricsPreview result={result} formData={formData} onUnlock={() => setStage(STAGES.output)} />
        )}
        {stage === STAGES.output && result && formData && (
          <MetricsOutput result={result} formData={formData} onRestart={handleRestart} />
        )}
      </div>
    </div>
  )
