...
export default function PRDOutput({ prd, formData, onRestart }) {
  ...
  const formatAsText = () => {
    ...
    text += `\nGenerated by kenrahu.com — Enterprise AI PRD Generator`
    return text
  }
        ...
        <div ref={outputRef} className="space-y-4">
          ...
          <div className="bg-surface border border-[#2A2A2A] rounded-xl p-6">
            ...
            <div className="flex flex-wrap gap-3 mt-3">
              <span className="text-xs bg-accent/10 text-accent px-2 py-1 rounded">Enterprise AI</span>
              <span className="text-xs bg-[#1A1A1A] text-muted px-2 py-1 rounded border border-[#2A2A2A]">Timeline: {formData.timeline}</span>
              {formData.complianceRequirements && (
                <span className="text-xs bg-[#1A1A1A] text-muted px-2 py-1 rounded border border-[#2A2A2A]">{formData.complianceRequirements}</span>
              )}
            </div>
          </div>
          ...
          {Object.entries(prd).map(([key, content]) => (
            <div key={key} className="bg-surface border border-[#2A2A2A] rounded-xl p-6">
              ...
              <div className="text-sm leading-relaxed whitespace-pre-line text-white/90">{content}</div>
            </div>
          ))}
          ...
          <div className="bg-surface border border-[#2A2A2A] rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-muted text-xs">Generated by kenrahu.com · Enterprise AI PRD Generator</p>
            <a
