    ...
    <div className="bg-surface border border-[#2A2A2A] rounded-xl p-5">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          <span className="text-2xl font-bold text-accent/40">#{uc.rank}</span>
          <h3 className="font-semibold text-sm leading-snug">{uc.name}</h3>
        </div>
        <span className={`text-xs font-medium px-2.5 py-1 rounded-full border flex-shrink-0 ${CATEGORY_STYLES[uc.category] || 'bg-surface text-muted border-[#2A2A2A]'}`}>
          {uc.category}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-[#2A2A2A]">
        ...
        <span className="text-accent font-bold">{uc.priorityScore?.toFixed(1)}</span>
      </div>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-bg px-6 py-16">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted text-sm">Top 2 use cases shown — enter your email to unlock the full matrix</p>
        </div>
        ...
        <div className="space-y-4 mb-4">
          {visible.map(uc => <UseCaseCard key={uc.name} uc={uc} />)}
        </div>
        ...
        {locked.length > 0 && (
          <div className="relative">
            <div className="space-y-4 blur-sm pointer-events-none select-none">
              {locked.map(uc => <UseCaseCard key={uc.name} uc={uc} />)}
            </div>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-bg/20 via-bg/80 to-bg rounded-xl px-6">
              <div className="bg-surface border border-[#2A2A2A] rounded-xl p-6 w-full max-w-md text-center">
                ...
                <h3 className="font-bold text-lg mb-1">Unlock the full matrix</h3>
